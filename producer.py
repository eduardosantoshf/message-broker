...
class Producer:
    def __init__(self, datatype):
        self.type = datatype
        self.queue = [middleware.PickleQueue(f"/{self.type}", middleware.MiddlewareType.PRODUCER)]
        if datatype == "temp":
            self.gen = self._temp
        elif datatype == "msg":
            self.gen = self._msg
        elif datatype == "weather":
            self.queue = [middleware.JSONQueue(f"/{self.type}/temperature/teste", middleware.MiddlewareType.PRODUCER), 
                          middleware.JSONQueue(f"/{self.type}/temperature", middleware.MiddlewareType.PRODUCER),
